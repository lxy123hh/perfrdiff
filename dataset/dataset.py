...
class ReactionDatasetTest(data.Dataset):
    def __init__(self, root_path, split, img_size=256, crop_size=224, clip_length=751, fps=25,
        ...
        self.use_person_specific_split = use_person_specific_split
        split_prefix = 'person_specific_' if self.use_person_specific_split else ''
        neighbour_prefix = 'person_specific_masked_' if self.use_person_specific_split else ''

        self._data_path = os.path.join(self._root_path, split_prefix + self._split)
        self._list_path = pd.read_csv(os.path.join(self._root_path, split_prefix + self._split + '.csv'),
                                      header=None, delimiter=',')
        self._list_path = self._list_path.drop(0)
        neighbour_emotion_path = os.path.join(
            root_path, neighbour_prefix + 'neighbour_emotion_' + split + '.npy')
...
def get_dataloader(conf):
    ...
    if conf.split in ["train", "val"]:  # train or validation
        ...
        dataset = ReactionDatasetTest(
            ...
            load_3dmm_l=conf.load_3dmm_l,
            load_ref=conf.load_ref,
            use_person_specific_split=conf.get("use_person_specific_split", True),
        )
