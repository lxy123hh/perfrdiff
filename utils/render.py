...
class Render(object):
    ...
    def rendering_for_fid(self, path, ind, listener_vectors, speaker_video_clip, listener_reference,
                          listener_video_clip, step=30, save_real=True):
        if step <= 0:
        ...
        c, h, w = listener_reference.shape
        total_frames = listener_vectors.shape[0]
        duration = max(1, total_frames // 20)
        listener_reference_frames = listener_reference.repeat(total_frames, 1, 1).view(total_frames, c, h, w)
        output_dict_list = []

        with torch.no_grad():
            for i in range(20):
                start = i * duration
                end = (i + 1) * duration if i != 19 else total_frames
                if start >= total_frames:
                    break
                listener_reference_copy = listener_reference_frames[start:end]
                semantics_copy = semantics[start:end]
                output_dict = self.pi_render(listener_reference_copy, semantics_copy)
                fake_videos = torch_img_to_np2(output_dict['fake_image'])
                output_dict_list.append(fake_videos)
        ...
        max_frames = min(listener_videos.shape[0], listener_video_clip.shape[0])
