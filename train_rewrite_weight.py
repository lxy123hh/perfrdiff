...
def train(cfg, model, data_loader, optimizer_hypernet, criterion, epoch, writer, device):
...
def main(args):
    ...
    optimizer_hypernet = optim.SGD(
        params=main_model.hypernet.parameters(),
    ...
    for epoch in range(cfg.trainer.start_epoch, cfg.trainer.epochs):
        train_loss, diff_prior_loss, diff_decoder_loss = train(
            cfg, main_model, data_loader, optimizer_hypernet, criterion, epoch, writer, device
        )

        logging.info(
            "Epoch: {} train_whole_loss: {:.5f} diff_prior_loss: {:.5f} diff_decoder_loss: {:.5f}".format(
                epoch + 1, train_loss, diff_prior_loss, diff_decoder_loss
            )
        )
