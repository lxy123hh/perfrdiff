...
``` bash
# Inference using Personalised GAFRG (with Weight Editing) for the legacy MAFRG metrics
python evaluate_rewrite_weight.py --mode test --config rewrite_weight.yaml

# Unified evaluator (default: PMAFRG; use --protocol both to emit MAFRG + PMAFRG in one run)
python evaluate_rewrite_weight_all.py --mode test --config configs/rewrite_weight.yaml --protocol both --compute_frrea
```
